 ...
 * Plugin URI: https://xnau.com/wordpress-plugins/members-database
 * Description: Plugin for managing a database of participants, members or volunteers
 * Author: Roland Barker, xnau webdesign
 * Version: 1.7.9.8
 * Author URI: https://xnau.com
 * License: GPL3
 * Text Domain: members-database
...
class Participants_Db extends PDb_Base {
   ...
   */
  const PLUGIN_NAME = 'members-database';
...
//    }
  ...
  public static function init()
  {
    ...
    self::$plugin_title = self::apply_filters( 'plugin_title', __( 'Participants Database', 'members-database' ) );
     ...
     * 
     * @version 1.6.3
     * @action members-database_initialized
     */
  ...
  public static function register_assets()
  {
     ...
     */
    wp_register_style( self::$prefix . 'frontend', plugins_url( '/css/members-database.css', __FILE__ ), array('dashicons'), self::$plugin_version );
...
//    wp_register_script( self::$prefix . 'jq-placeholder', plugins_url( 'js/jquery.placeholder.min.js', __FILE__ ), array('jquery') );
    ...
    wp_register_style( self::$prefix . 'frontend', plugins_url( '/css/members-database.css', __FILE__ ), null, self::$plugin_version );
...
//          "record" => __("Do you really want to delete the selected record?", 'members-database' ),
//          "records" => __("Do you really want to delete the selected records?", 'members-database' ),
//      ));
//      wp_enqueue_script(self::$prefix.'list-admin');
    }

    if ( false !== stripos( $hook, 'members-database_settings_page' ) ) {
      wp_enqueue_script( self::$prefix . 'settings_script' );
    }

    if ( false !== stripos( $hook, 'members-database-edit_participant' ) ) {
      //wp_enqueue_script(self::$prefix.'record_edit_script');
    }

    if ( false !== stripos( $hook, 'members-database-manage_fields' ) ) {
      ...
      wp_localize_script( self::$prefix . 'manage_fields', 'PDb_L10n', array(
          ...
          'must_remove' => '<h4>' . __( 'You must remove all fields from the {name} group before deleting it.', 'members-database' ) . '</h4>',
          /* translators: don't translate the words in brackets {} */
          'delete_confirm' => '<h4>' . __( 'Delete the "{name}" {thing}?', 'members-database' ) . '</h4>',
          'unsaved_changes' => __( "The changes you made will be lost if you navigate away from this page.", 'members-database' ),
          'datatype_confirm' => '<h4 class="dashicons-before dashicons-info warning">' . __( 'Changing the form element on a field that has stored data can result in data loss.', 'members-database' ) .'</h4><p><a href="https://wp.me/p48Sj5-Zb" target="_blank">' . __( 'More information here…', 'members-database' ) . '</a></p>',
          'datatype_confirm_button' => __( 'Yes, change the form element', 'members-database' ),
          'datatype_cancel_button' => __( 'No, don\'t change the form element', 'members-database' ),
      ) );
...
//self::$validation_errors->add_error('', 'success');
  ...
  public static function _make_uploads_dir( $dir = '' )
  {
    ...
    if ( mkdir( Participants_Db::base_files_path() . $dir, 0755, true ) === false ) {
      ...
      if ( is_object( self::$validation_errors ) )
        self::$validation_errors->add_error( '', sprintf( __( 'The uploads directory (%s) could not be created.', 'members-database' ), $dir ) );
  ...
  public static function get_admin_record_link( $id )
  {
    ...
    $path = 'admin.php?page=members-database-edit_participant&action=edit&id=' . $id;
...
//session: '.print_r(Participants_Db::$session,1));
  ...
  private static function _set_i18n()
  {
    ...
    self::$i18n = array(
        'submit' => __( 'Submit', 'members-database' ),
        'apply' => __( 'Apply', 'members-database' ),
        'next' => __( 'Next', 'members-database' ),
        'previous' => __( 'Previous', 'members-database' ),
        'updated' => __( 'The record has been updated.', 'members-database' ),
        'added' => __( 'The new record has been added.', 'members-database' ),
        'zero_rows_error' => __( 'No record was added on query: %s', 'members-database' ),
        'database_error' => __( 'Database Error: %2$s on query: %1$s', 'members-database' )
    );
  }
  ...
  public static function add_body_class( $classes )
  {
    ...
    if ( !empty( $shortcodes ) ) {
      $classes[] = 'members-database-shortcode';
  ...
  {
    global $pagenow;
    if ( ($pagenow == 'admin.php' && filter_input( INPUT_GET, 'page' ) === 'members-database_settings_page') || $pagenow == 'options.php' ) {
      /*
  ...
  public static function plugin_footer()
  {
    ...
    <div id="PDb_footer" class="pdb-footer widefat redfade postbox">
      <div class="section">
        <h4><?php echo 'Participants Database ', self::$plugin_version ?><br /><?php _e( 'WordPress Plugin', 'members-database' ) ?></h4>
        <p><em><?php _e( 'Helping organizations manage their volunteers, members and participants.', 'members-database' ) ?></em></p>
      </div>
      <div class="section">
        <h4><a class="glyph-link" href="http://xnau.com"><span class="icon-xnau-glyph"></span></a><?php _e( 'Developed by', 'members-database' ) ?><br /><a href="http://xnau.com">xn<span class="lowast">&lowast;</span>au webdesign</a></h4>
        <p><?php _e( 'Suggestions or criticisms of this plugin? I&#39;d love to hear them: email ', 'members-database' ) ?><a href="mailto:support@xnau.com">support@xnau.com.</a>
      </div>
      <div class="section">
        <p><?php printf( __( 'Please consider contributing to the continued support and development of this software by visiting %1$sthis plugin&#39;s page,%3$s giving the plugin a %2$srating%3$s or review, or dropping something in the %1$stip jar.%3$s Thanks!', 'members-database' ), '<a href="http://xnau.com/wordpress-plugins/members-database#donation-link">', '<a href="http://wordpress.org/extend/plugins/members-database/">', '</a>' ) ?></p>
      </div>
    </div>
    <?php
    endif;
  ...
  public static function plugin_label( $key )
  {
    $labels = self::apply_filters('plugin_labels', array(
        'add_participant' => __( 'Add Participant', 'members-database' ),
        'list_participants' => __( 'List Participants', 'members-database' ),
        'manage_fields' => __( 'Manage Database Fields', 'members-database' ),
        'upload_csv' => __( 'Import CSV File', 'members-database' ),
        'plugin_settings' => '<span class="dashicons dashicons-admin-generic"></span>' . __( 'Settings', 'members-database' ),
        'setup_guide' => __( 'Setup Guide', 'members-database' ),
        'add_record_title' => __( 'Add New Participant Record', 'members-database' ),
        'edit_record_title' => __( 'Edit Existing Participant Record', 'members-database' ),
        'list_participants_title' => __( 'List Participants', 'members-database' ),
        'export_csv_title' => __( 'Export CSV', 'members-database' ),
        'manage_list_columns' => __( 'Manage List Columns', 'members-database' ),
    ) );
  ...
  {
    return array_merge( $links, array('settings' => '<a href="' . admin_url( 'admin.php?page=members-database_settings_page' ) . '">' . __( 'Settings', 'members-database' ) . '</a>') );
  }
...
//      $links[] = '<a href="http://wordpress.org/support/view/plugin-reviews/members-database">' . __( 'Submit a rating or review', 'members-database' ) . ' </a>';
//      $links[] = '<span style="color:#6B4001;">' . __( 'Free tech support and continued development relies on your support:', 'members-database' ) . ' <a class="button xnau-contribute" href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=6C7FSX2DQFWY4">' . __( 'contribute', 'members-database' ) . '</a></span>';
    }
    return $links;
  }
...
{
  echo '<div class="notice notice-error is-dismissible"><p><span class="dashicons dashicons-warning"></span>' . sprintf( __( 'Participants Database requires PHP version %s to function properly, you have PHP version %s. Please upgrade PHP. The Plugin has been auto-deactivated.', 'members-database' ), Participants_Db::min_php_version, PHP_VERSION ) . '</p></div>';
