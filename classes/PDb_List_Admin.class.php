...
class PDb_List_Admin {
  ...
  public static function initialize()
  {
    ...
    $apply_confirm_messages = Participants_Db::apply_filters( 'admin_list_with_selected_action_conf_messages', array(
                'delete' => array(
                    "singular" => __( "Do you really want to delete the selected record?", 'members-database' ),
                    "plural" => __( "Do you really want to delete the selected records?", 'members-database' ),
                ),
                'approve' => array(
                    "singular" => __( "Approve the selected record?", 'members-database' ),
                    "plural" => __( "Approve the selected records?", 'members-database' ),
                ),
                'unapprove' => array(
                    "singular" => __( "Unapprove the selected record?", 'members-database' ),
                    "plural" => __( "Unapprove the selected records?", 'members-database' ),
                ),
                'send_signup_email' => array(
                    "singular" => __( "Send the signup email to the selected record?", 'members-database' ),
                    "plural" => __( "Send the signup email to the selected records?", 'members-database' ),
                ),
                'send_resend_link_email' => array(
                    "singular" => __( 'Send the "resend link" email to the selected record?', 'members-database' ),
                    "plural" => __( 'Send the "resend link" email to the selected records?', 'members-database' ),
                ),
                'recipient_count_exceeds_limit' => sprintf( __( 'The number of selected records exceeds the %s email send limit.%s Only the first %s will be sent.', 'members-database'), '<a href="https://xnau.com/product_support/email-expansion-kit/#email_session_send_limit" target="_blank" >', '</a>', '{limit}'),
                    )
    );
...
//        'wrap_tag' => '<div class="pdb-list"><div class="pagination"><label>' . _x('Page', 'noun; page number indicator', 'members-database') . ':</label> ',
          ...
          switch ( $selected_action ) {
            ...
            case 'delete':
              ...
              if ( $selected_count > 0 ) {
                ...
                if ( $result > 0 ) {
                  Participants_Db::set_admin_message( __( 'Record delete successful.', 'members-database' ), 'updated' );
                }
            ...
            case 'unapprove':
              if ( $selected_count > 0 ) {
                ...
                if ( $result > 0 ) {
                  ...
                  Participants_Db::set_admin_message( Participants_Db::apply_filters('admin_list_action_feedback', sprintf( _x( 'Approval status for %d records has been updated.', 'number of records with approval statuses set', 'members-database' ), $selected_count ) ), 'updated' );
                }
                $last_query = $wpdb->last_query;
              }
              break;
            ...
            case 'send_signup_email':
              ...
              $message = sprintf( _nx( '%d email was sent.', '%d emails were sent.', $send_count, 'number of emails sent', 'members-database' ), $send_count );
    ...
    <div id="pdb-list-admin"   class="wrap participants_db">
      ...
      <div id="poststuff">
        ...
        private static function _sort_filter_forms()
        {
                ...
                <tbody><tr><td>
                      ...
                      for ( $i = 0; $i <= $filter_count - 1; $i++ ) :
                        ...
                        <fieldset class="widefat inline-controls">
                          <?php if ( $i === 0 ): ?>
                            <legend><?php _e( 'Show only records with', 'members-database' ) ?>:</legend>
                            <?php
                          endif;
                          ...
                          _ex( 'that', 'joins two search terms, such as in "Show only records with last name that is Smith"', 'members-database' );
                          $element = array(
                              'type' => 'dropdown',
                              'name' => 'operator[' . $i . ']',
                              ...
                              'options' => array(
                                  'null_select' => false,
                                  __( 'is', 'members-database' ) => '=',
                                  __( 'is not', 'members-database' ) => '!=',
                                  __( 'contains', 'members-database' ) => 'LIKE',
                                  __( 'doesn&#39;t contain', 'members-database' ) => 'NOT LIKE',
                                  __( 'is greater than', 'members-database' ) => 'gt',
                                  __( 'is less than', 'members-database' ) => 'lt',
                              ),
                          );
                          PDb_FormElement::print_element( $element );
                          ?>
                          ...
                          if ( $i < $filter_count - 1 ) {
                            ...
                            $element = array(
                                ...
                                'options' => array(
                                    __( 'and', 'members-database' ) => 'AND',
                                    __( 'or', 'members-database' ) => 'OR',
                                ),
                            );
                          } else {
                            $element = array(
                      ...
                      <fieldset class="widefat inline-controls">
                        ...
                        <div class="widefat inline-controls filter-count">
                          <label for="list_filter_count"><?php _e( 'Number of filters to use: ', 'members-database' ) ?><input id="list_filter_count" name="list_filter_count" class="number-entry single-digit" type="number" max="5" min="1" value="<?php echo $filter_count ?>"  /></label>
                        </div>
                      </fieldset>
                    </td></tr><tr><td>
                      <fieldset class="widefat inline-controls">
                        <legend><?php _e( 'Sort by', 'members-database' ) ?>:</legend>
                        <?php
                        $element = array(
                            'type' => 'dropdown',
                            'name' => 'sortBy',
          ...
          <h3><?php printf( _n( '%s record found, sorted by: %s.', '%s records found, sorted by: %s.', self::$num_records, 'members-database' ), self::$num_records, Participants_Db::column_title( self::$filter['sortBy'] ) ) ?></h3>
          <?php
        }
...
//            do_action(Participants_Db::$prefix . 'admin_list_form_top', $this);
            ...
            $with_selection_actions = array(
                        __( 'approve', 'members-database' ) => 'approve',
                        __( 'unapprove', 'members-database' ) => 'unapprove',
                    );
            if ( current_user_can( Participants_Db::plugin_capability( 'plugin_admin_capability', 'delete participants' ) ) ) {
              $with_selection_actions = array(
                        __( 'delete', 'members-database' ) => 'delete'
            ...
            <table class="form-table"><tbody><tr><td>
                    <fieldset class="widefat inline-controls">
                      ...
                      ?>
                      <?php printf( __( 'Show %s items per page.', 'members-database' ), $list_limit ) ?>
              ...
              <div class="pdb-horiz-scroll-scroller">
                ...
                <table class="wp-list-table widefat fixed pages pdb-list stuffbox" cellspacing="0" >
                      ...
                      foreach ( self::$participants as $value ) {
                        ...
                        <tr>
                          ...
                          <td>
                            ...
                            <?php endif ?>
                            <a href="admin.php?page=<?php echo 'members-database' ?>-edit_participant&amp;action=edit&amp;id=<?php echo $value['id'] ?>" title="<?php _e( 'Edit', 'members-database' ) ?>"><span class="dashicons dashicons-edit"></span></a>
                          </td>
                          <?php
                  ...
                  <?php else : // if there are no records to show; do this
                    ...
                    <tbody>
                      <tr>
                        <td><?php _e( 'No records found', 'members-database' ) ?></td>
                      </tr>
                    </tbody>
                  <?php
            ...
            <div class="inside">
              ...
              <form method="post" class="csv-export">
                ...
                <fieldset class="inline-controls">
                  <?php _e( 'File Name', 'members-database' ) ?>:
                  <input type="text" name="filename" value="<?php echo $suggested_filename ?>" size="<?php echo $namelength ?>" />
                  <input type="submit" name="submit-button" value="<?php _e( 'Download CSV for this list', 'members-database' ) ?>" class="button button-primary" />
                  <label for="include_csv_titles"><input type="checkbox" name="include_csv_titles" value="1"><?php _e( 'Include field titles', 'members-database' ) ?></label>
                </fieldset>
                <p>
                  <?php _e( 'This will download the whole list of participants that match your search terms, and in the order specified by the sort. The export will include records on all list pages. The fields included in the export are defined in the "CSV" column on the Manage Database Fields page.', 'members-database' ) ?>
                </p>
              </form>
            </div>
          </div>
...
';
    ...
    <th scope="col" style="width:3em">
      <?php if ( self::user_can_use_with_selected() ) : ?>
        <?php /* translators: uses the check symbol in a phrase that means "check all"  printf('<span class="checkmark" >&#10004;</span>%s', __('all', 'members-database'))s */ ?>
  ...
  {
    $prefix = __('PDb Admin List With Selected: ', 'members-database');
    $admin_list_events = array(
        'pdb-list_admin_with_selected_delete' => $prefix . __( 'delete', 'members-database' ),
        'pdb-list_admin_with_selected_approve' => $prefix . __( 'approve', 'members-database' ),
        'pdb-list_admin_with_selected_unapprove' => $prefix . __( 'unapprove', 'members-database' ),
        'pdb-list_admin_with_selected_send_signup_email' => $prefix . __( 'send signup email', 'members-database' ),
    );
...
';
  ...
  private static function _setup_i18n()
  {
    ...
    self::$i18n = array(
        ...
        'apply' => __( 'Apply' ),
        'with_selected' => _x( 'With selected', 'phrase used just before naming the action to perform on the selected items', 'members-database' ),
    );
  }
