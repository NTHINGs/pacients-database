...
class PDb_Manage_Fields {
  ...
  protected function print_header()
  {
    ...
    <div class="wrap participants_db">
      ...
      <h3><?php _e( 'Manage Database Fields', 'members-database' ) ?></h3>
      <?php Participants_Db::admin_message(); ?>
      <h4><?php _e( 'Field Groups', 'members-database' ) ?>:</h4>
      <div id="fields-tabs">
        <ul>
          <?php
          ...
          echo '<li class="utility"><a href="#field_groups">' . __( 'Field Groups', 'members-database' ) . '</a>' . $mask . '</li>';
          echo '<li class="utility"><a href="#help">' . __( 'Help', 'members-database' ) . '</a>' . $mask . '</li>';
          ?>
        </ul>
        <?php
      }
                ...
                <table class="wp-list-table widefat fixed manage-fields" >
                  ...
                  <tbody id="<?php echo $group ?>_fields">
                    ...
                    if ( $num_group_rows < 1 ) { // there are no rows in this group to show
                      ?>
                      <tr><td colspan="<?php echo count( $this->attribute_columns[$group] ) + 1 ?>"><?php _e( 'No fields in this group', 'members-database' ) ?></td></tr>
                      <?php
                    } else {
                      ...
                      foreach ( $this->fields_data[$group] as $database_row ) :
                        ...
                        <tr id="db_row_<?php echo $database_row['id'] ?>">
                          <?php if ( !$internal_group ) : ?>
                            ...
                            if ( !$internal_group ) :
                              ?>
                              <a href="javascript:return false" title="<?php echo $database_row['id'] ?>" data-thing-name="delete_<?php echo $database_row['id'] ?>" class="delete" data-thing="<?php _e( 'field', 'members-database' ) ?>"><span class="dashicons dashicons-no"></span></a>
                            </td>
                            <?php
                          endif; // internal group test
      ...
      protected function print_group_edit_tab_content()
      {
        ...
        <div id="field_groups" class="manage-fields-wrap">
          <form id="manage_field_groups" method="post">
            ...
            <h3><?php _e( 'Edit / Add / Remove Field Groups', 'members-database' ) ?></h3>
            <p>
              <?php
            ...
            <table class="wp-list-table widefat fixed manage-fields manage-field-groups" >
              <thead>
                <tr>
                  <th scope="col" class="fields vertical-title"><span><?php echo $this->table_header( __( 'fields', 'members-database' ) ) ?></span></th>
                  <th scope="col" class="delete vertical-title"><span><?php echo $this->table_header( __( 'delete', 'members-database' ) ) ?></span></th>
                  <?php
              ...
              <tbody>
                ...
                foreach ( $this->group_values as $group => $group_values ) {
                  ...
                  <tr>
                    ...
                    <td>
                      <a href="<?php echo $group_count ?>" data-thing-name="delete_<?php echo $group ?>" class="delete" data-thing="<?php _e( 'group', 'members-database' ) ?>"><span class="dashicons dashicons-no"></span></a>
                    </td>
                    <?php
    ...
    protected function process_submit()
    {
        ...
        case $this->i18n['add field']:
          ...
          if ( in_array( $atts['name'], Participants_Db::$reserved_names ) ) {
            ...
            Participants_Db::set_admin_message( sprintf(
                    '<strong>%s</strong> %s: %s', 
                    __( 'Cannot add a field with that name', 'members-database' ), 
                    __( 'This name is reserved; please choose another. Reserved names are', 'members-database' ), implode( ', ', Participants_Db::$reserved_names ) 
                    ), 'error' );
            break;
          }
          ...
          if ( array_key_exists( $atts['name'], Participants_Db::$fields ) ) {
            ...
            Participants_Db::set_admin_message( sprintf(
                    '<strong>%s</strong> %s', 
                    __( 'Cannot add a field with that name', 'members-database' ), 
                    __( 'The name must be unique: a field with that name has been previously defined.', 'members-database' ) 
                    ), 'error' );
            break;
          }
          ...
          if ( preg_match( '/^(\d)/', $atts['name'] ) ) {
            ...
            Participants_Db::set_admin_message( sprintf(
                    '<strong>%s</strong> %s', 
                    __( 'The name cannot begin with a number', 'members-database' ), 
                    __( 'Please choose another.', 'members-database' )
                    ), 'error' );
            break;
          }
          ...
          // prevent name from beginning with a number
          if ( $result ) {
            Participants_Db::set_admin_message( __( 'The new field was added.', 'members-database' ), 'update' );
          } else {
            Participants_Db::set_admin_message( __( 'The field could not be added.', 'members-database' ), 'error' );
          }
      ...
      if ( $result === false ) {
        if ( $wpdb->last_error ) {
          ...
          Participants_Db::set_admin_message( __( 'There was an error; the settings were not saved.', 'members-database' ), 'error' );
        }
      } elseif ( is_int( $result ) ) {
        Participants_Db::set_admin_message( __( 'Your information has been updated', 'members-database' ), 'updated' );
      }
    }
    ...
    function parse_db_error( $error, $context )
    {
      ...
      if ( $item && false !== stripos( $error, 'duplicate' ) ) {
        ...
        $message = sprintf( __( 'The %1$s was not added. There is already a %1$s with that name, please choose another.', 'members-database' ), $item );
      }
    ...
    public static function get_i18n()
    {
      return array(
          ...
          'update fields' => __( 'Update Fields', 'members-database' ),
          'update groups' => __( 'Update Groups', 'members-database' ),
          'add field' => __( 'Add Field', 'members-database' ),
          'add group' => __( 'Add Group', 'members-database' ),
          'group' => __( 'group', 'members-database' ),
          'field' => __( 'field', 'members-database' ),
          'new field title' => __( 'new field title', 'members-database' ),
          'new group title' => __( 'new group title', 'members-database' ),
          'fields' => _x( 'Fields', 'column name', 'members-database' ),
          'Group' => _x( 'Group', 'column name', 'members-database' ),
          'order' => _x( 'Order', 'column name', 'members-database' ),
          'name' => _x( 'Name', 'column name', 'members-database' ),
          'title' => _x( 'Title', 'column name', 'members-database' ),
          'default' => _x( 'Default', 'column name', 'members-database' ),
          'help_text' => _x( 'Help Text', 'column name', 'members-database' ),
          'form_element' => _x( 'Form Element', 'column name', 'members-database' ),
          'values' => _x( 'Values', 'column name', 'members-database' ),
          'validation' => _x( 'Validation', 'column name', 'members-database' ),
          'display_column' => str_replace( ' ', '<br />', _x( 'Display Column', 'column name', 'members-database' ) ),
          'admin_column' => str_replace( ' ', '<br />', _x( 'Admin Column', 'column name', 'members-database' ) ),
          'sortable' => _x( 'Sortable', 'column name', 'members-database' ),
          'CSV' => _x( 'CSV', 'column name, acronym for "comma separated values"', 'members-database' ),
          'persistent' => _x( 'Persistent', 'column name', 'members-database' ),
          'signup' => _x( 'Signup', 'column name', 'members-database' ),
          'readonly' => _x( 'Read Only', 'column name', 'members-database' ),
          'admin' => _x( 'Admin', 'column name', 'members-database' ),
          'delete' => _x( 'Delete', 'column name', 'members-database' ),
          'display' => _x( 'Display', 'column name', 'members-database' ),
          'description' => _x( 'Description', 'column name', 'members-database' ),
      );
    }
