...
//     error_log(__METHOD__.' incoming shorcode atts:'.print_r($shortcode_atts,1));
  ...
  protected function _print_from_template()
  {
    if ( false === $this->template ) {
      ...
      $this->output = '<p class="alert alert-error">' . sprintf( _x( '<%1$s>The template %2$s was not found.</%1$s> Please make sure the name is correct and the template file is in the correct location.', 'message to show if the plugin cannot find the template', 'members-databes' ), 'strong', $this->template ) . '</p>';
  ...
  public function print_next_button()
  {
    if ( strlen( $this->submission_page ) > 0 ) {
      printf( '<a type="button" class="button button-secondary" href="%s" >%s</a>', $this->submission_page, __( 'next', 'members-databes' ) );
    }
  }
